...
import React, { useState } from "react";
import { Link } from "react-router-dom";
import { Menu, X, CircleUser, Heart, ShoppingCart, Package, LogOut ,User } from "lucide-react";

const UserSidebar = ({ onCollapse }) => {
  const [isOpen, setIsOpen] = useState(true);

  return (
    <div className={`fixed top-18 left-0 h-screen bg-gray-900 text-white transition-all duration-300 ${isOpen ? "w-64" : "w-16"}`}>
      {/* Sidebar Toggle Button */}
      <button 
        className="p-3 text-white hover:bg-gray-800 w-full flex justify-end" 
        onClick={() => setIsOpen(!isOpen)}
      ...
      <nav className="mt-4">
        <ul>
          <li>
            <Link to="/user-details" className="flex items-center p-3 hover:bg-gray-700 transition">
            <User size={20} />
              <span className={`ml-3 ${!isOpen ? "hidden" : ""}`}>User Details</span>
            </Link>
          </li>
          <li>
            <Link to="/me" className="flex items-center p-3 hover:bg-gray-700 transition">
              <CircleUser size={20} />
